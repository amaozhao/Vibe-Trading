...
- **2026-08-16** 🔧 **Anthropic runs no longer die on recovery, and symbol search stops reporting empty results as healthy**: Three recovery paths appended `{"role": "system"}` messages mid-conversation, which the Anthropic Messages API rejects, so a run that hit a content-filter skip or grounding recovery died on the next call with "Received multiple non-consecutive system messages"; recovery steering now travels as user messages with inline `<system>` tags ([#1112](https://github.com/HKUDS/Vibe-Trading/pull/1112), closes [#1109](https://github.com/HKUDS/Vibe-Trading/issues/1109)). `search_symbol` answered ticker+name queries such as `XOM ExxonMobil` with zero candidates while both sources reported `ok`, so no identity was ever locked and every downstream data tool refused with `identity_required`; the Yahoo path now marks that query shape `skipped` instead of a misleading `ok` ([#1114](https://github.com/HKUDS/Vibe-Trading/pull/1114), closes [#1108](https://github.com/HKUDS/Vibe-Trading/issues/1108)). Also: `LANGCHAIN_REASONING_EFFORT` was silently dropped on the Anthropic branch, so Fable 5, Opus 5 and Sonnet 5 always ran at the default effort — it is now forwarded through a positive model allowlist, because models that reject the parameter fail with a hard 400 ([#1115](https://github.com/HKUDS/Vibe-Trading/pull/1115)); the Tencent A-share loader recovers from `CERTIFICATE_VERIFY_FAILED` by trusting the certifi CA bundle instead of the incomplete default store ([#1113](https://github.com/HKUDS/Vibe-Trading/pull/1113)); SEC filers without a literal `GrossProfit` XBRL tag now get the documented `revenue - cogs` derivation that was dead code, restoring gross margin ([#1111](https://github.com/HKUDS/Vibe-Trading/pull/1111)); and swarm workers truncate oversized tool results with the same shared helper and notice as the main loop, so a sub-agent can no longer be handed a partial result with no indication anything was cut ([#1110](https://github.com/HKUDS/Vibe-Trading/pull/1110)). Thanks [@lorenzozanee](https://github.com/lorenzozanee), [@straun-repo](https://github.com/straun-repo), [@x-lambda](https://github.com/x-lambda), [@cgycorey](https://github.com/cgycorey), and [@Shizoqua](https://github.com/Shizoqua)!
