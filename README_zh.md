...
- **2026-08-16** 🔧 **Anthropic 运行不再死在恢复路径上，symbol 搜索不再把空结果报成正常**：三条恢复路径会在对话中途追加 `{"role": "system"}` 消息，而 Anthropic Messages API 拒绝这种形式，因此一旦触发内容过滤跳过或 grounding 恢复，运行就会在下一次调用时以 "Received multiple non-consecutive system messages" 崩溃；恢复引导现在改由带内联 `<system>` 标签的用户消息传递（[#1112](https://github.com/HKUDS/Vibe-Trading/pull/1112)，关闭 [#1109](https://github.com/HKUDS/Vibe-Trading/issues/1109)）。`search_symbol` 对 `XOM ExxonMobil` 这类 ticker+名称组合查询返回零候选，而两个来源都报告 `ok`，导致身份永远无法锁定、下游所有数据工具都以 `identity_required` 拒绝；Yahoo 路径现在把这类查询形态标记为 `skipped`，而不是误导性的 `ok`（[#1114](https://github.com/HKUDS/Vibe-Trading/pull/1114)，关闭 [#1108](https://github.com/HKUDS/Vibe-Trading/issues/1108)）。此外：`LANGCHAIN_REASONING_EFFORT` 在 Anthropic 分支被静默丢弃，Fable 5、Opus 5 与 Sonnet 5 始终以默认档位运行——现在通过正向模型白名单转发，因为拒绝该参数的模型会以硬 400 直接失败（[#1115](https://github.com/HKUDS/Vibe-Trading/pull/1115)）；腾讯 A 股行情源改用 certifi 的 CA 证书包，从 `CERTIFICATE_VERIFY_FAILED` 中恢复（[#1113](https://github.com/HKUDS/Vibe-Trading/pull/1113)）；没有字面 `GrossProfit` XBRL 标签的 SEC 财报现在能拿到文档承诺的 `revenue - cogs` 推导（此前是死代码），毛利率恢复正确（[#1111](https://github.com/HKUDS/Vibe-Trading/pull/1111)）；swarm worker 用与主循环相同的共享截断助手与提示处理超大工具结果，子代理不会再拿到没有任何截断说明的半截结果（[#1110](https://github.com/HKUDS/Vibe-Trading/pull/1110)）。感谢 [@lorenzozanee](https://github.com/lorenzozanee)、[@straun-repo](https://github.com/straun-repo)、[@x-lambda](https://github.com/x-lambda)、[@cgycorey](https://github.com/cgycorey) 和 [@Shizoqua](https://github.com/Shizoqua)！
