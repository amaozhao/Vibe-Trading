...
- **2026-08-16** 🔧 **Las ejecuciones con Anthropic ya no mueren en las rutas de recuperación, y la búsqueda de símbolos deja de reportar resultados vacíos como si fueran correctos**: Tres rutas de recuperación añadían mensajes `{"role": "system"}` a mitad de conversación, y la API Messages de Anthropic los rechaza, así que una ejecución que tocaba un salto del filtro de contenido o una recuperación de evidencia moría en la siguiente llamada con "Received multiple non-consecutive system messages"; el direccionamiento de recuperación viaja ahora como mensajes de usuario con etiquetas `<system>` en línea ([#1112](https://github.com/HKUDS/Vibe-Trading/pull/1112), cierra [#1109](https://github.com/HKUDS/Vibe-Trading/issues/1109)). `search_symbol` respondía a consultas de ticker+nombre como `XOM ExxonMobil` con cero candidatos mientras ambas fuentes reportaban `ok`, de modo que la identidad nunca se bloqueaba y todas las herramientas de datos posteriores rechazaban la petición con `identity_required`; la vía de Yahoo marca ahora esa forma de consulta como `skipped` en vez de un `ok` engañoso ([#1114](https://github.com/HKUDS/Vibe-Trading/pull/1114), cierra [#1108](https://github.com/HKUDS/Vibe-Trading/issues/1108)). Además: `LANGCHAIN_REASONING_EFFORT` se descartaba en silencio en la rama de Anthropic, así que Fable 5, Opus 5 y Sonnet 5 funcionaban siempre con el esfuerzo por defecto — ahora se reenvía a través de una lista de modelos permitida, porque los modelos que rechazan el parámetro fallan con un 400 directo ([#1115](https://github.com/HKUDS/Vibe-Trading/pull/1115)); el cargador de acciones A de Tencent se recupera de `CERTIFICATE_VERIFY_FAILED` confiando en el paquete CA de certifi en lugar del almacén por defecto incompleto ([#1113](https://github.com/HKUDS/Vibe-Trading/pull/1113)); los declarantes de la SEC sin una etiqueta XBRL literal `GrossProfit` reciben ahora la derivación documentada `revenue - cogs`, que era código muerto, restaurando el margen bruto ([#1111](https://github.com/HKUDS/Vibe-Trading/pull/1111)); y los trabajadores del swarm recortan los resultados de herramientas sobredimensionados con el mismo helper y aviso compartidos del bucle principal, de modo que un subagente ya no recibe un resultado parcial sin indicación de que algo se cortó ([#1110](https://github.com/HKUDS/Vibe-Trading/pull/1110)). ¡Gracias a [@lorenzozanee](https://github.com/lorenzozanee), [@straun-repo](https://github.com/straun-repo), [@x-lambda](https://github.com/x-lambda), [@cgycorey](https://github.com/cgycorey) y [@Shizoqua](https://github.com/Shizoqua)!
