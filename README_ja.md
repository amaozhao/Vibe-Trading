...
- **2026-08-16** 🔧 **Anthropic の実行がリカバリー経路で停止しなくなり、シンボル検索が空結果を正常と報告しなくなった**：3 つのリカバリー経路が会話の途中に `{"role": "system"}` メッセージを追記していましたが、Anthropic Messages API はこれを拒否するため、コンテンツフィルターのスキップや grounding リカバリーに当たった実行は次の呼び出しで "Received multiple non-consecutive system messages" により停止していました。リカバリー指示はインラインの `<system>` タグ付きユーザーメッセージとして送られるようになりました（[#1112](https://github.com/HKUDS/Vibe-Trading/pull/1112)、[#1109](https://github.com/HKUDS/Vibe-Trading/issues/1109) をクローズ）。`search_symbol` は `XOM ExxonMobil` のようなティッカー＋企業名のクエリにゼロ候補を返しながら両ソースが `ok` を報告していたため、identity がロックされず、後段のデータツールがすべて `identity_required` で拒否されていました。Yahoo 経路はこのクエリ形状を誤解を招く `ok` ではなく `skipped` と報告するようになりました（[#1114](https://github.com/HKUDS/Vibe-Trading/pull/1114)、[#1108](https://github.com/HKUDS/Vibe-Trading/issues/1108) をクローズ）。ほかにも：`LANGCHAIN_REASONING_EFFORT` は Anthropic ブランチで黙って捨てられており、Fable 5・Opus 5・Sonnet 5 は常にデフォルトの effort で動いていました。パラメータを拒否するモデルはハードな 400 で失敗するため、現在はポジティブなモデル許可リストを通じて転送されます（[#1115](https://github.com/HKUDS/Vibe-Trading/pull/1115)）。Tencent の A 株ローダーは certifi の CA バンドルを信頼することで `CERTIFICATE_VERIFY_FAILED` から回復します（[#1113](https://github.com/HKUDS/Vibe-Trading/pull/1113)）。リテラルの `GrossProfit` XBRL タグを持たない SEC 提出書類には、ドキュメントに記載されていたのに死にコードだった `revenue - cogs` の導出が適用され、粗利率が復旧します（[#1111](https://github.com/HKUDS/Vibe-Trading/pull/1111)）。swarm worker はメインループと同じ共有ヘルパーと通知で大きすぎるツール結果を切り詰めるため、サブエージェントが切り詰めの表示なしに部分結果を受け取ることはなくなりました（[#1110](https://github.com/HKUDS/Vibe-Trading/pull/1110)）。[@lorenzozanee](https://github.com/lorenzozanee)、[@straun-repo](https://github.com/straun-repo)、[@x-lambda](https://github.com/x-lambda)、[@cgycorey](https://github.com/cgycorey)、[@Shizoqua](https://github.com/Shizoqua) に感謝します！
