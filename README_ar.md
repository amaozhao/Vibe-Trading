...
- **2026-08-16** 🔧 **لم تعد عمليات Anthropic تموت في مسارات التعافي، ولم يعد بحث الرموز يبلّغ عن نتائج فارغة وكأنها سليمة**: كانت ثلاثة مسارات تعافي تُلحق رسائل `{"role": "system"}` في منتصف المحادثة، وواجهة Anthropic Messages ترفض ذلك، فكانت أي عملية تشغيل تصطدم بتخطّي مرشح المحتوى أو بتعافي الإسناد تموت عند الاستدعاء التالي برسالة "Received multiple non-consecutive system messages"؛ صار توجيه التعافي الآن يسري كرسائل مستخدم بوسوم `<system>` مضمّنة ([#1112](https://github.com/HKUDS/Vibe-Trading/pull/1112)، يغلق [#1109](https://github.com/HKUDS/Vibe-Trading/issues/1109)). وكان `search_symbol` يجيب عن استعلامات "رمز + اسم" مثل `XOM ExxonMobil` بصفر مرشحين بينما يبلّغ المصدران عن `ok`، فلا تنقفل الهوية أبداً وترفض كل أدوات البيانات اللاحقة الطلب برمز `identity_required`؛ أصبح مسار Yahoo الآن يعلّم هذا الشكل من الاستعلامات بـ `skipped` بدل `ok` مضلل ([#1114](https://github.com/HKUDS/Vibe-Trading/pull/1114)، يغلق [#1108](https://github.com/HKUDS/Vibe-Trading/issues/1108)). كذلك: كان `LANGCHAIN_REASONING_EFFORT` يُسقط بصمت في فرع Anthropic، فتعمل Fable 5 وOpus 5 وSonnet 5 دائماً بالمستوى الافتراضي — وهو الآن يُمرَّر عبر قائمة سماح موجبة للنماذج، لأن النماذج الرافضة للمعامل تفشل بخطأ 400 صريح ([#1115](https://github.com/HKUDS/Vibe-Trading/pull/1115))؛ ويتعافى محمّل أسهم A الخاص بـ Tencent من `CERTIFICATE_VERIFY_FAILED` بالثقة بحزمة شهادات certifi بدل المخزن الافتراضي الناقص ([#1113](https://github.com/HKUDS/Vibe-Trading/pull/1113))؛ ويحصل مقدّمو تقارير SEC من دون وسم `GrossProfit` XBRL الحرفي على اشتقاق `revenue - cogs` الموثّق الذي كان كوداً ميتاً، فتعود هوامش الربح الإجمالية إلى الصواب ([#1111](https://github.com/HKUDS/Vibe-Trading/pull/1111))؛ ويقصّ عمال السرب نتائج الأدوات الضخمة بنفس المساعد والإشعار المشتركين مع الحلقة الرئيسية، فلا يتلقى الوكيل الفرعي نتيجة جزئية من دون أي إشارة إلى أنها اقتُطعت ([#1110](https://github.com/HKUDS/Vibe-Trading/pull/1110)). شكراً [@lorenzozanee](https://github.com/lorenzozanee), [@straun-repo](https://github.com/straun-repo), [@x-lambda](https://github.com/x-lambda), [@cgycorey](https://github.com/cgycorey), و [@Shizoqua](https://github.com/Shizoqua)!
